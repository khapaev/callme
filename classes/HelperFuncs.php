...
class HelperFuncs
{
	...
	/**
	 * Метод поиска и возврата внутреннего номера пользователя
	 * 
	 * @param int $phoneInner
	 * 
	 * @return int phone inner or false
	 */
	public function userPhoneInnerByPhoneInners($phoneInner)
	{
		$result = $this->getBitrixApi(
			array(
				"FILER" => array(
					"UF_PHONE_INNER_MULTIPLE" => $phoneInner
				)
			),
			'user.get'
		);
		...
		switch ($disposition) {
			...
			default:
				if (empty($disposition)) {
					$sipcode = 304; // Если пустой пришел, то поставим неотвечено
				} else {
					$sipcode = 603; // Отклонено, когда все остальное
				}
				break;
		}
	...
	public function getCrmContactNameByExtNum($extNum)
	{
		...
		if ($result) {
			if (isset($result['total']) && $result['total'] > 0) {
				$FullName = $this->translit($result['result'][0]['NAME'] . '_' . $result['result'][0]['LAST_NAME']);
			}
		}
